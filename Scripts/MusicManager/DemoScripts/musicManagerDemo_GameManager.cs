...
public class musicManagerDemo_GameManager : MonoBehaviour {
	...
	void Update () {
		if (player.GetComponent<musicManagerDemo_PlayerScript>().inZone1)
		{
			if(!repeat1){
		...
		if (player.GetComponent<musicManagerDemo_PlayerScript>().inZone2)
		{
			if(!repeat2){
		...
		if (player.GetComponent<musicManagerDemo_PlayerScript>().inZone3)
		{
			if(!repeat3){
